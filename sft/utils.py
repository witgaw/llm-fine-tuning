...
def seed_from_saved(saved):
    # I don't think this is crucial, but it might save some headscratching if comparing results calculated from saved model vs those obtained during a live run, this way evaluations downstream from training should start from same random numbers as during training
    random.seed(saved["system_rng"])
    np.random.get_state(saved["numpy_rng"])
    torch.cuda.set_rng_state(saved["torch_rng"])
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.deterministic = True


def get_device(use_gpu, use_mps, verbose):
