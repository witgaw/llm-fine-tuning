...
def _compute_hidden_state_to_token(hidden_state, embedding_weights):
    return torch.matmul(hidden_state, embedding_weights.t())


def _process_last_hidden_state(last_hidden_state, attention_mask):
    ...
    return {"last_hidden_state": last_hidden_state, "last_token": last_token}
...
class GPT2Model(GPTPreTrainedModel):
    ...
    def embed(self, input_ids):
        ...
        output_embeds = inputs_embeds + pos_embeds
        self.embed_dropout(output_embeds)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        return _process_last_hidden_state(sequence_output, attention_mask)
    ...
    def hidden_state_to_token(self, hidden_state):
        ...
        """
        return _compute_hidden_state_to_token(hidden_state, self.word_embedding.weight)
    ...
    def from_pretrained(cls, model="gpt2", d=768, l=12, num_heads=12):
        ...
        for i in range(l):
            ...
            l.self_attention.key.weight.data = gpt_model.state_dict()[f"h.{i}.attn.c_attn.weight"][
                :, d : d * 2
            ].T
            l.self_attention.key.bias.data = gpt_model.state_dict()[f"h.{i}.attn.c_attn.bias"][
                d : d * 2
            ]
            l.self_attention.value.weight.data = gpt_model.state_dict()[
                f"h.{i}.attn.c_attn.weight"
            ][:, d * 2 :].T
            l.self_attention.value.bias.data = gpt_model.state_dict()[f"h.{i}.attn.c_attn.bias"][
                d * 2 :
            ]
